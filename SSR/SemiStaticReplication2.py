...
def continuation_q(w_1, b_1, w_2, b_2, stock, delta_t):
    ...
    cont += np.sum(w_2[mask1_indices] * (w_1[mask1_indices] *
                                         np.tile(stock[:, None], len(mask1_indices)) + np.exp(-r*delta_t)*b_1[
...
def model(S0, K, mu, sigma, N, monitoring_dates, style, optimizer, hidd_nds, l1, l2):
    ...
    early_stopping = EarlyStopping(monitor='val_loss', mode='min', patience=5, verbose=0, start_from_epoch=20)
    ...
    rlnn = pre_training(rlnn, early_stopping, sample_pathsS[:, 1] / normalizer, option_pff, batch,
                        n_epochs, val_ratio)
    ...
    rlnn.fit(sample_pathsS[:, M] / normalizer, option[:, M], epochs=3000, batch_size=int(N / 10), verbose=0,
    ...
    for m in range(M - 1, 0, -1):
        normalizer = np.mean(sample_pathsS[:, m])
        rlnn.fit(sample_pathsS[:, m] / normalizer, option[:, m], epochs=1000, batch_size=int(N / 10), verbose=0,
                 validation_split=0.3, callbacks=[early_stopping])
...
def upper_bound(r, trained_weights, stock_paths, strike, monitoring, style, nodes_n):
    ...
    for m in range(1, n_mon):
        ...
        q = continuation_q(weights_layer_1, biases_layer_1, weights_layer_2, biases_layer_2,
                           stock_paths[:, m - 1] / normalizer, differences[m - 1])
...
def lower_bound(r, trained_weights, stock_paths, strike, monitoring, style):
    ...
    for m in range(n_mon - 1):
        ...
        weights_layer_2 = np.array(current_weights[2]).reshape(-1)
        biases_layer_2 = np.array(current_weights[3])
        q = continuation_q(weights_layer_1, biases_layer_1, weights_layer_2, biases_layer_2, s / normalizer, differences[m])
    ...
    for j in range(sample_size):
        ...
        discounted_values[j] = h_of_s[j] * np.exp(-r * monitoring[tau[j]])
...
weights, option_value = model(S, K_strike, mu, sigma, N, monitoring_dates, pf_style,
                              tf.keras.optimizers.legacy.Adamax(learning_rate=0.001), nodes, 5e-4, 1e-3)
